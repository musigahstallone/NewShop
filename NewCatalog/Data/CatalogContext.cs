...
{
    private static readonly Func<CatalogContext, int?, int?, int?, int, IAsyncEnumerable<CatalogItem>> GetCatalogItemsQuery =
       EF.CompileAsyncQuery((CatalogContext context, int? catalogBrandId, int? before, int? after, int pageSize) =>
          context.CatalogItem.AsNoTracking()
                 .OrderBy(ci => ci.Id)
