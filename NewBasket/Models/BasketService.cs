    ...
    : Basket.BasketBase
{
    ...
    public override async Task<CustomerBasketResponse> UpdateBasket(CustomerBasketRequest request, ServerCallContext context)
    {
        ...
        var response = await repository.UpdateBasketAsync(customerBasket) ?? throw new RpcException(new Status(StatusCode.NotFound, $"Basket with buyer id {request.BuyerId} does not exist"));
